...
const Home = ({ token, searchResults, isAdmin }) => {
    ...
    <>
      <Stack direction="horizontal" gap={3} className="full-height">
        <div className="left-section p-2">
          <Ingredients token={token} isAdmin={isAdmin}/>
        </div>
        <div className="vr" />
        <div className="center-section p-2">
          {selectedRecipe ? (
            <RecipeDetails token={token} recipe={selectedRecipe} isAdmin={isAdmin}/>
          ) : (
        ...
        <div className="right-section p-2">
          <Recipes token={token} onRecipeSelect={handleSelectedRecipe} searchResults={searchResults} isAdmin={isAdmin}/>
        </div>
      </Stack>
    </>
  );
